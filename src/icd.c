...
/* set estimation type */
	...
	int metal_detection=0;
	if(metal_detection){
		int Nr = sinogram->geom_info.Nr;
		int Nc = sinogram->geom_info.Nc;
		int Nv = sinogram->geom_info.Nv;



        	fprintf(stdout,"before metal detection \n");
		fflush(stdout);
		struct Image metal_img;
		readImgInfo(info_recon_dir,&(metal_img.img_info));
		initImage(&(sinogram->geom_info), &metal_img, 0.0);
		fillImgInfo(&(metal_img.img_info));
	
        	if(PnP_mode){	
		for (i =0; i < image->img_info.Nx; i++){
		for(j=0;j<image->img_info.Ny;j++){
			for(int k=0;k<image->img_info.Nz;k++){
				if(consensus_X->img[i*image->img_info.Ny *image->img_info.Nz + j*image->img_info.Nz+k]>0.0384){ //1000 HU
					metal_img.img[i*image->img_info.Ny *image->img_info.Nz + j*image->img_info.Nz+k]=consensus_X->img[i*image->img_info.Ny *image->img_info.Nz + j*image->img_info.Nz+k];
				}	
			}
		}
		}	
		...
		for(j=0;j<image->img_info.Ny;j++){
			for(int k=0;k<image->img_info.Nz;k++){
				if(Vmean->img[i*image->img_info.Ny *image->img_info.Nz + j*image->img_info.Nz+k]>0.0384){ //1000 HU
					metal_img.img[i*image->img_info.Ny *image->img_info.Nz + j*image->img_info.Nz+k]=Vmean->img[i*image->img_info.Ny *image->img_info.Nz + j*image->img_info.Nz+k];
				}	
			}
		}
		}	
		...
		metal_forward(e, metal_img.img, recon_mask, &(sinogram->geom_info), &(image->img_info),&stored_view_xy_info,myid,NUMPROCS);

		fprintf(stdout,"before MPI send and receive \n");
		fflush(stdout);
		if((myid/(sinogram->geom_info.num_focal_spots*sinogram->geom_info.num_sources))!=0){
			MPI_Send(&e[0],Nr*Nc*Nv,MPI_FLOAT,myid%(sinogram->geom_info.num_focal_spots*sinogram->geom_info.num_sources),myid,MPI_COMM_WORLD);
		}
		else
		{
			ENTRY*  full_sino_data = (ENTRY *)get_spc(Nr*Nc*Nv*(NUMPROCS/(sinogram->geom_info.num_focal_spots *sinogram->geom_info.num_sources)), sizeof(ENTRY));
			ENTRY* sino_stack= (ENTRY *)get_spc(Nr*Nc*Nv*(NUMPROCS/(sinogram->geom_info.num_focal_spots *sinogram->geom_info.num_sources)), sizeof(ENTRY));
		
			memcpy(&sino_stack[0],&e[0],sizeof(ENTRY)*Nr*Nc*Nv);

			fprintf(stdout,"myid %d before MPI_Recv \n",myid);

			for(i=1;i<(NUMPROCS/(sinogram->geom_info.num_focal_spots*sinogram->geom_info.num_sources));i++){
				MPI_Recv(&sino_stack[(Nr*Nc*Nv*i)],Nr*Nc*Nv,MPI_FLOAT,i*(sinogram->geom_info.num_focal_spots*sinogram->geom_info.num_sources)+myid,i*(sinogram->geom_info.num_focal_spots*sinogram->geom_info.num_sources)+myid,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
			}

			fprintf(stdout,"myid %d after MPI_Recv \n",myid);
			fflush(stdout);	
	
			for(i=0;i<Nv ;i++){
			for(j=0;j<(NUMPROCS/(sinogram->geom_info.num_focal_spots*sinogram->geom_info.num_sources));j++){
				memcpy(&full_sino_data[i*(NUMPROCS/(sinogram->geom_info.num_focal_spots *sinogram->geom_info.num_sources))*Nr*Nc + j*Nr*Nc],&sino_stack[Nr*Nc*Nv*j+Nr*Nc*i],sizeof(ENTRY)*Nr*Nc);
			...
			strcpy(name, "");
        		sprintf(suffix, "metal.vjk");
        		strcat(name, fname);
       		 	strcat(name, suffix);
        		writeImage(name, &metal_img);
			strcpy(name, "");
        		sprintf(suffix, "metal_sinogram_id_%d",myid);
			strcat(name, fname);
        		strcat(name, suffix);
			writeSinogram_float(name, full_sino_data, sinogram->geom_info.Nr, sinogram->geom_info.Nc, sinogram->geom_info.Nv*(NUMPROCS/(sinogram->geom_info.num_focal_spots *sinogram->geom_info.num_sources)));
			strcpy(name, "");
        		sprintf(suffix, "_sinostack_%d",myid);
			strcat(name, fname);
        		strcat(name, suffix);
			writeSinogram_float(name, sino_stack, sinogram->geom_info.Nr, sinogram->geom_info.Nc, sinogram->geom_info.Nv*(NUMPROCS/(sinogram->geom_info.num_focal_spots *sinogram->geom_info.num_sources)));
			
			free((void *)full_sino_data);
			free((void *)sino_stack);
		}
		freeImage(&metal_img);
	}
